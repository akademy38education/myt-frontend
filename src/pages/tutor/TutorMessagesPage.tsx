...
import { useConversationMessages, useTutorConversations, useSendMessage, messagingService } from "@/features/messaging";
import { initials, formatRelativeToNow } from "@/utils/formatters";
import { cn } from "@/utils/cn";

export function TutorMessagesPage() {
  const { user } = useAuth();
  const { data: conversations, isLoading } = useTutorConversations(user?.id ?? "");
  ...
  const filtered = conversations?.filter((c) => c.participantName.toLowerCase().includes(query.toLowerCase())) ?? [];
  ...
  if (isLoading) return <LoadingState label="Loading your messages..." />;

  if (!conversations || conversations.length === 0) {
    return <EmptyState icon={MessageSquare} title="No conversations yet" description="Once a student books a lesson with you, you'll be able to message them here." />;
  }

  return (
    <Card className="overflow-hidden">
      <div className="grid h-[32rem] sm:grid-cols-[16rem_1fr]">
